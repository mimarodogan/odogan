...
final class PostController
{
    ...
    public function store(Request $req): Response
    {
        ...
        // Tag attach (comma-separated input)
        $tagInput = (string) $req->input('tags', '');
        if ($tagInput !== '') {
            $names = array_filter(array_map('trim', explode(',', $tagInput)));
            Tag::syncForPost($id, $names);
        }
    ...
    public function update(Request $req, array $args): Response
    {
        ...
        // Tag attach (form'dan gelen comma-separated)
        $tagInput = (string) $req->input('tags', '');
        $names = $tagInput !== '' ? array_filter(array_map('trim', explode(',', $tagInput))) : [];
        Tag::syncForPost((int) $post['id'], $names);
     ...
     * Actions: publish, draft, archive, delete, change_category, add_tag
    ...
    public function bulk(Request $req): Response
    {
        ...
        $allowed = ['publish', 'draft', 'archive', 'delete', 'change_category', 'add_tag'];
        ...
        foreach ($ids as $pid) {
            ...
            try {
                if ($action === 'delete') {
                    ...
                    $tagName = trim((string) $req->input('new_tag', ''));
                    if ($tagName === '') { $skipped++; continue; }
                    $tag = \App\Models\Tag::findOrCreate($tagName);
                    if ($tag) {
                        // post_tag pivot'a INSERT IGNORE — mevcut etiketleri silmez
                        \App\Core\Database::instance()->run(
                            'INSERT IGNORE INTO post_tag (post_id, tag_id) VALUES (:pid, :tid)',
                            [':pid' => $pid, ':tid' => (int) $tag['id']]
                        );
                        $applied++;
                    } else {
                        $skipped++;
