...
final class AuthController
{
    ...
    public function verifyTotp(Request $req): Response
    {
        ...
        // Recovery code denemesi (TOTP fail ettiyse) — encrypted veya legacy plaintext JSON
        $stored = $user['totp_recovery_codes'] ?? null;
        $storedDecrypted = is_string($stored)
            ? \App\Services\Crypto::decryptIfEncrypted($stored)
            : (is_array($stored) ? json_encode($stored) : '');
        $recovery = $storedDecrypted !== '' ? (json_decode($storedDecrypted, true) ?: []) : [];
        if (is_array($recovery) && $recovery) {
            $remaining = TotpService::consumeRecoveryCode($recovery, $code);
            if ($remaining !== null) {
                $remainingJson = json_encode(array_values($remaining), JSON_UNESCAPED_UNICODE);
                User::update($userId, [
                    'totp_recovery_codes' => \App\Services\Crypto::encrypt((string) $remainingJson),
                ]);
                AuthService::completeTotpLogin($userId);
                RateLimiter::clear('totp:ip:' . $ip);
