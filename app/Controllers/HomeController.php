...
final class HomeController
{
    public function index(Request $req): Response
    {
        ...
        // Sözlük widget — "Bugün öğren" 3 rastgele terim
        $glossaryPicks = [];
        if (function_exists('feature') && feature('glossary_enabled')) {
            $glossaryPicks = $cache->remember(
                'home:glossary-picks:' . date('Y-m-d'),
                3600,  // günde değişir
                fn() => \App\Models\Glossary::randomPicks(3),
                ['home', 'glossary']
            );
        }
