...
// FAQ: yeni format faq_json'dan oku. Geriye dönük uyum: bazı eski
// girdilerde FAQ definition HTML'inde "Sıkça Sorulan Sorular" H2 olarak
// gömülü olabilir — bunu strip et ki çift görünmesin.
$faqs = [];
if (!empty($item['faq_json'])) {
    $faqs = \App\Services\FaqService::decode((string) $item['faq_json']);
}
// Legacy: definition'da hâlâ "Sıkça Sorulan Sorular" H2 bloğu varsa kaldır.
// (Yeni AI üreteci HTML'e FAQ koymaz; bu sadece eski girdiler için.)
$definition = (string) preg_replace(
    '/<h2[^>]*>\s*(?:Sıkça\s+Sorulan\s+Sorular|SSS|S\.S\.S)\s*<\/h2>.*?(?=<h2[^>]*>|$)/siu',
    '',
    $definition
);
...
<article class="post post-glossary">
    ...
    <?php if (!empty($faqs)): ?>
    <section class="post-glossary-faq" aria-labelledby="faq-title">
        <h2 id="faq-title">Sıkça Sorulan Sorular</h2>
        <div class="faq-accordion">
            <?php foreach ($faqs as $i => $f): ?>
                <details class="faq-item"<?= $i === 0 ? ' open' : '' ?>>
                    <summary><?= esc((string) $f['q']) ?></summary>
                    <div class="faq-answer">
                        <?= nl2br(esc((string) $f['a'])) ?>
                    </div>
                </details>
            <?php endforeach; ?>
        </div>
    ...
    <?php if (!empty($posts_with_term ?? [])): ?>
    <section class="post-glossary-backlinks" aria-labelledby="bktitle">
        <h2 id="bktitle">Bu Terimin Geçtiği Yazılar</h2>
        <ul class="backlink-list">
            <?php foreach ($posts_with_term as $bp):
                $bpUrl = url('/' . $bp['category_slug'] . '/' . $bp['slug']);
            ?>
                <li>
                    <a href="<?= esc($bpUrl) ?>" title="<?= esc($bp['title']) ?>">
                        <strong><?= esc($bp['title']) ?></strong>
                    </a>
                    <?php if (!empty($bp['excerpt'])): ?>
                        <p class="muted"><?= esc(mb_substr((string) $bp['excerpt'], 0, 140)) ?>…</p>
                    <?php endif; ?>
                </li>
            <?php endforeach; ?>
        </ul>
