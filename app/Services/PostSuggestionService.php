 ...
 * paylaşılan tag sayısı ile skorla, en alakalı yazıları döndür.
 *
...
final class PostSuggestionService
{
    ...
    public static function findSimilar(string $body, ?int $excludePostId = null, int $limit = 5): array
    {
        ...
        // Excluded post + tag-bazlı bonus skor
        $excludeTagIds = [];
        if ($excludePostId !== null) {
            $excludeTagIds = self::tagIdsOf($excludePostId);
        }
        $suggestions = [];
        foreach ($candidates as $c) {
            ...
            // Tag bonus — paylaşılan tag başına +0.5 puan
            if ($excludeTagIds) {
                $candTagIds = self::tagIdsOf($pid);
                $shared = count(array_intersect($excludeTagIds, $candTagIds));
                $score += $shared * 0.5;
            }
            $suggestions[] = [
                'id' => $pid,
    ...
    /**
     * Bir yazının tag id'lerini getir. Tablo yoksa boş array.
     *
     * @return int[]
     */
    private static function tagIdsOf(int $postId): array
    {
        try {
            $rows = Database::instance()->fetchAll(
                'SELECT tag_id FROM post_tag WHERE post_id = :pid',
                [':pid' => $postId]
            );
